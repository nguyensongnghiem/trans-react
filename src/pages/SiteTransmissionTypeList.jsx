...
function SiteTransmissionTypeList() {
    ...
    const filteredList = types.filter((item) =>
        item.name.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const validationSchema = Yup.object({
        name: Yup.string().required("Tên loại truyền dẫn là bắt buộc"),
    });
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
                ...
                <table className="w-full min-w-max table-auto text-left">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="p-4">
                                <Typography variant="small" color="blue-gray" className="font-bold">Tên Loại Truyền Dẫn</Typography>
                            </th>
                            {isAdmin && (
            ...
            <Dialog open={openCreate} handler={() => setOpenCreate(!openCreate)} size="xs">
                <DialogHeader>Thêm mới Loại Truyền Dẫn</DialogHeader>
                <Formik
                    initialValues={{ name: "" }}
                            ...
                            <DialogBody>
                                <div className="flex flex-col gap-4">
                                    <div>
                                        <Field name="name">
                                            {({ field }) => (
                                                <Input {...field} label="Tên Loại" error={touched.name && Boolean(errors.name)} />
                                            )}
                                        </Field>
            ...
            <Dialog open={openEdit} handler={() => setOpenEdit(!openEdit)} size="xs">
                <DialogHeader>Cập nhật Loại Truyền Dẫn</DialogHeader>
                {selectedItem && (
                    <Formik
            ...
            <Dialog open={openDelete} handler={() => setOpenDelete(!openDelete)} size="xs">
                ...
                <DialogBody>
                    Bạn có chắc chắn muốn xóa loại truyền dẫn <b>{selectedItem?.name}</b> không?
                </DialogBody>
                <DialogFooter>
...
export default SiteTransmissionTypeList;