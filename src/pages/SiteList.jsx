...
function SiteList() {
  ...
  const getAllSites = async () => {
    const sites = await siteService.searchSites(page, searchTerm.siteId, searchTerm.transOwner, searchTerm.transType);
    setSiteList(sites)
    setIsLoading(false)
  };
  ...
  return (
    <div className="container px-3">
      ...
      <button onClick={openModal} className="mb-3 w-full rounded-sm bg-sky-500 px-2 py-1 font-semibold text-white shadow-md hover:cursor-pointer hover:bg-sky-600 md:w-fit">
      ...
      <div className="container max-w-full">
        ...
        >
          <Form className="mb-3 flex flex-col justify-start gap-5 md:flex-row md:flex-wrap md:items-end">
          ...
          <table className="min-w-full table-auto border-collapse border-2">
            <thead>
              <tr className="bg-gray-200 uppercase text-gray-600">
                ...
                <th className="px-6 py-3 text-center">Site ID</th>
                <th className="px-6 py-3 text-center">Site ID khác</th>
                <th className="px-6 py-3 text-center">Vĩ độ</th>
                <th className="px-6 py-3 text-center">Kinh độ</th>
                <th className="px-6 py-3 text-center">Đơn vị sở hữu TD</th>
                <th className="px-6 py-3 text-center">Loại TD</th>
              ...
              {siteList.content.map((site) => {
                return (
                  <tr key={site.id} className="border-b border-gray-200 hover:bg-sky-100">
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.province.name}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.siteId}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.siteId2}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.latitude}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.longitude}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.transmissionOwner?.name}</td>
                    <td className="whitespace-nowrap px-6 py-3 text-center">{site.siteTransmissionType?.name}</td>
      ...
      >
        <h2 className="mb-3 text-lg font-bold text-red-500" ref={(_subtitle) => (subtitle = _subtitle)}>Xác nhận xóa thông tin</h2>
        <p className="mb-3">Xác nhận xóa trạm khỏi cơ sở dữ liệu ?</p>
        <div className="flex justify-between gap-3">
          <button
            className="mb-3 inline-block rounded-sm bg-slate-500 px-2 py-1 font-semibold text-white shadow-md hover:cursor-pointer hover:bg-slate-600"
          >Hủy</button>
