...
import useFoConnectionTypes from "../hooks/useFoConnectionTypes";

function FoConnectionTypeList() {
    const {
        foConnectionTypes,
        isLoading: isFetching,
        createFoConnectionType,
        updateFoConnectionType,
        deleteFoConnectionType,
        fetchFoConnectionTypes,
    } = useFoConnectionTypes();
    ...
    const handleCreate = async (values, { resetForm }) => {
        const success = await createFoConnectionType(values);
        if (success) {
            setOpenCreate(false);
            resetForm();
        }
    ...
    const handleUpdate = async (values) => {
        const success = await updateFoConnectionType(selectedItem.id, values);
        if (success) {
            setOpenEdit(false);
    ...
    const handleDelete = async () => {
        ...
        const success = await deleteFoConnectionType(selectedItem.id);
        if (success) {
            setOpenDelete(false);
            setSelectedItem(null);
        }
    ...
    const filteredList = foConnectionTypes.filter((item) =>
        item.name.toLowerCase().includes(searchTerm.toLowerCase())
    );

    const validationSchema = Yup.object({
        name: Yup.string().required("Tên hình thức kết nối là bắt buộc"),
    });
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
                ...
                <table className="w-full min-w-max table-auto text-left">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="p-4">
                                <Typography variant="small" color="blue-gray" className="font-bold">Tên hình thức</Typography>
                            </th>
                            <th className="p-4">
            ...
            >
                <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-3">
                    <div>
                        <Typography variant="h5" color="blue-gray" className="font-semibold text-gray-900">
                            Thêm mới Hình thức kết nối
                        </Typography>
                        <Typography className="text-xs font-normal text-gray-500 mt-0.5">
                            Nhập thông tin hình thức kết nối mới
                        </Typography>
                    </div>
                    <IconButton
                        size="sm"
                ...
                <Formik
                    initialValues={{ name: "", description: "", colorCode: "#000000", lineStyle: "solid" }}
                                        ...
                                        <Field name="name">
                                            {({ field }) => (
                                                <Input
                                                    ...
                                                    placeholder="Nhập tên hình thức kết nối..."
            ...
            >
                <div className="flex items-center justify-between border-b border-gray-200 bg-gray-50 px-4 py-3">
                    <div>
                        <Typography variant="h5" color="blue-gray" className="font-semibold text-gray-900">
                            Cập nhật Hình thức kết nối
                        </Typography>
                        <Typography className="text-xs font-normal text-gray-500 mt-0.5">
                            Chỉnh sửa thông tin hình thức kết nối #{selectedItem?.id}
                        </Typography>
                    </div>
                    <IconButton
                        size="sm"
            ...
            >
                ...
                <DialogBody className="p-6 text-blue-gray-700">
                    <Typography variant="paragraph" color="blue-gray" className="font-medium">
                        Bạn có chắc chắn muốn xóa hình thức kết nối <b>{selectedItem?.name}</b> không?
                    </Typography>
...
export default FoConnectionTypeList;
