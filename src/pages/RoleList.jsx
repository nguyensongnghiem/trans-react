...
function RoleList() {
    ...
    const handleUpdate = async (values) => {
        try {
            await axiosInstance.put(`roles/${selectedItem.id}`, values);
    ...
    const handleDelete = async () => {
        try {
            await axiosInstance.delete(`roles/${selectedItem.id}`);
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Tổng số: <span className="font-semibold text-blue-600">{filteredList.length}</span> vai trò
                    </p>
                </div>
                {isAdmin && (
                    <CustomButton
                ...
                <table className="w-full min-w-max table-auto text-left">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="p-4">
                                <Typography variant="small" color="blue-gray" className="font-bold">Tên vai trò</Typography>
                            </th>
                            {isAdmin && (
            ...
            <Dialog open={openCreate} handler={() => setOpenCreate(!openCreate)} size="sm">
                <DialogHeader>Thêm mới Vai trò</DialogHeader>
                <Formik
                    initialValues={{ name: "" }}
                    ...
                    {({ errors, touched }) => (
                        <Form>
                            <DialogBody>
                                <Field name="name">
                                    {({ field }) => (
                                        <Input {...field} label="Tên vai trò" error={touched.name && Boolean(errors.name)} />
                                    )}
                                </Field>
            ...
            <Dialog open={openEdit} handler={() => setOpenEdit(!openEdit)} size="sm">
                <DialogHeader>Cập nhật Vai trò</DialogHeader>
                {selectedItem && (
                    <Formik
            ...
            <Dialog open={openDelete} handler={() => setOpenDelete(!openDelete)} size="xs">
                ...
                <DialogBody>
                    Bạn có chắc muốn xóa vai trò <b>{selectedItem?.name}</b>?
                </DialogBody>
                <DialogFooter>
