...
const BackupScheduleManagement = () => {
  ...
  const fetchSchedules = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.get("/backup/schedules");
  ...
  const fetchRegions = async () => {
    try {
      // Giả định backend Java đã proxy request này tới service backup
      const response = await axiosInstance.get("/backup/regions");
  ...
  const handleDelete = async (scheduleId) => {
    ...
    try {
      await axiosInstance.delete(`/backup/schedules/${scheduleId}`);
    ...
    try {
      if (isEdit) {
        await axiosInstance.put(
          `/backup/schedules/${currentScheduleId}`,
          payload
        );
        toast.success("Cập nhật lịch backup thành công!");
      } else {
        await axiosInstance.post("/backup/schedules", payload);
...
export default BackupScheduleManagement;
