    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
            ...
            <Dialog open={openCreate} handler={() => setOpenCreate(!openCreate)} size="sm">
                <DialogHeader>Thêm mới Đơn vị truyền dẫn</DialogHeader>
                <Formik
                    initialValues={{ name: "" }}
                    ...
                    {({ errors, touched }) => (
                        <Form>
                            <DialogBody>
                                <Field name="name">
                                    {({ field }) => (
                                        <Input {...field} label="Tên đơn vị" error={touched.name && Boolean(errors.name)} />
                                    )}
                                </Field>
            ...
            <Dialog open={openEdit} handler={() => setOpenEdit(!openEdit)} size="sm">
                <DialogHeader>Cập nhật Đơn vị truyền dẫn</DialogHeader>
                {selectedItem && (
                    <Formik
            ...
            <Dialog open={openDelete} handler={() => setOpenDelete(!openDelete)} size="xs">
                ...
                <DialogBody>
                    Bạn có chắc muốn xóa đơn vị <b>{selectedItem?.name}</b>?
                </DialogBody>
                <DialogFooter>
