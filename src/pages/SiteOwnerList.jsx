...
function SiteOwnerList() {
    ...
    const handleCreate = async (values, { resetForm }) => {
        try {
            await axiosInstance.post("siteOwners", values);
    ...
    const handleUpdate = async (values) => {
        try {
            await axiosInstance.put(`siteOwners/${selectedItem.id}`, values);
    ...
    const handleDelete = async () => {
        try {
            await axiosInstance.delete(`siteOwners/${selectedItem.id}`);
    ...
    const validationSchema = Yup.object({
        name: Yup.string().required("Tên chủ trạm là bắt buộc"),
    });
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen font-sans">
            <div className="flex justify-between items-center mb-6">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Quản lý Chủ trạm</h1>
                    <p className="text-sm text-gray-500 mt-1">
                        Tổng số: <span className="font-semibold text-blue-600">{filteredList.length}</span> chủ trạm
                    </p>
                </div>
                {isAdmin && (
                    <CustomButton
                ...
                <table className="w-full min-w-max table-auto text-left">
                    <thead>
                        <tr className="bg-gray-50 border-b border-gray-200">
                            ...
                            <th className="p-4">
                                <Typography variant="small" color="blue-gray" className="font-bold">Tên chủ trạm</Typography>
                            </th>
                            {isAdmin && (
            ...
            <Dialog open={openCreate} handler={() => setOpenCreate(!openCreate)} size="sm">
                <DialogHeader>Thêm mới Chủ trạm</DialogHeader>
                <Formik
                    initialValues={{ name: "" }}
                    ...
                    {({ errors, touched }) => (
                        <Form>
                            <DialogBody>
                                <Field name="name">
                                    {({ field }) => (
                                        <Input {...field} label="Tên chủ trạm" error={touched.name && Boolean(errors.name)} />
                                    )}
                                </Field>
                                <ErrorMessage name="name" component="div" className="text-red-500 text-xs mt-1" />
                            </DialogBody>
                            <DialogFooter>
                                <Button variant="text" onClick={() => setOpenCreate(false)} className="mr-2">Hủy</Button>
            ...
            <Dialog open={openEdit} handler={() => setOpenEdit(!openEdit)} size="sm">
                <DialogHeader>Cập nhật Chủ trạm</DialogHeader>
                {selectedItem && (
                    <Formik
                    ...
                    >
                        {({ errors, touched }) => (
                            <Form>
                                ...
                                <DialogFooter>
                                    <Button variant="text" onClick={() => setOpenEdit(false)} className="mr-2">Hủy</Button>
            ...
            <Dialog open={openDelete} handler={() => setOpenDelete(!openDelete)} size="xs">
                ...
                <DialogBody>
                    Bạn có chắc muốn xóa chủ trạm <b>{selectedItem?.name}</b>?
                </DialogBody>
                <DialogFooter>
